...
#include <time.h>        // For NTP-based Unix timestamps
...
/*
 * Sync time from NTP server so lastSeen is a real Unix timestamp.
...
void initFirebaseSync() {
    ...
    // Set device as online with real Unix timestamp
    time_t now;
    time(&now);
    firebasePatch("devices/" + String(DEVICE_ID), 
        "{\"online\":true,\"lastSeen\":" + String((unsigned long)now) + ",\"ip\":\"" + WiFi.localIP().toString() + "\"}");
...
void sendHeartbeat() {
    ...
    time(&now);
    firebasePatch("devices/" + String(DEVICE_ID),
        "{\"online\":true,\"lastSeen\":" + String((unsigned long)now) + "}");
}
